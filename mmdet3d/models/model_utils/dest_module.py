...
def box_parametrization_to_sides(box_center_unnorm, box_size, box_angle):
    box_center_upright = flip_axis_to_camera_tensor(box_center_unnorm)
    boxes = get_3d_box_batch_tensor(box_size, box_angle, box_center_upright)
    boxes = convert_corners_camera2lidar(boxes)
    box_sides = boxes[:,:,[4, 2]]
...
class ISSMDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", num_proposal=256, use_biscan=False, last_layer=False):
        super().__init__()
    ...
    def forward(self, query, key, query_pos, key_pos, self_posembed=None, issm_posembed=None):
        """
        :param query: B C Pq
        :param key: B C Pk
        ...
        if self_posembed is not None:
            query_pos_embed = self_posembed(query_pos).permute(0, 2, 1)
        else:
            query_pos_embed = None
        if issm_posembed is not None:
            key_pos_embed = issm_posembed(key_pos).permute(0, 2, 1)
        else:
            key_pos_embed = None
        ...
        query = self.self_attn(q.transpose(0,1), k.transpose(0,1), value=v.transpose(0,1)).transpose(0,1)
...
class Serialization(nn.Module):
    def __init__(
        ...
        self.template = torch.load(f'/ssd/wcx/codes/fully_sup_detection/Group-Free-3D/utils/hilbert/curve_template_3d_rank_{self.bit}.pth')
