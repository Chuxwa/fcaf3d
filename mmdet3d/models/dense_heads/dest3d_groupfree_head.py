...
from mmdet3d.models.model_utils.dest_module import ISSMDecoderLayer, Serialization
EPS = 1e-6
...
class DEST3DHeadV1(nn.Module):
    ...
    def __init__(self,
                 ...
                 semantic_loss=None,
                 angle_loss=None):
        super(DEST3DHeadV1, self).__init__()
        ...
        for _ in range(self.num_decoder_layers):
            self.decoder_issm_posembeds.append(
                build_positional_encoding(decoder_issm_posembeds))
    ...
    def init_weights(self):
        ...
        for m in self.decoder_issm_posembeds.parameters():
            if m.dim() > 1:
                nn.init.xavier_uniform_(m)
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        for i in range(self.num_decoder_layers):
            ...
            query, key_seri = self.decoder_layers[i](query, key_seri, query_pos, key_pos_seri, 
                                                     self_posembed=self.decoder_self_posembeds[i], 
                                                     issm_posembed=self.decoder_issm_posembeds[i])
            if not self.decoder_layers[i].last_layer:
    ...
    def loss(self,
        ...
        (sampling_targets, sampling_weights, assigned_size_targets,
         ...
         box_loss_weights, valid_gt_weights, _) = targets
    ...
    def get_targets(self,
        ...
        (sampling_targets, assigned_size_targets, size_class_targets,
         ...
         objectness_masks, assignment) = multi_apply(self.get_targets_single, points,
        ...
        return (sampling_targets, sampling_weights, assigned_size_targets,
                ...
                objectness_weights, box_loss_weights, valid_gt_weights, assignment)
    ...
    def get_targets_single(self,
        ...
        return (sampling_targets, assigned_size_targets, size_class_targets,
                ...
                objectness_targets, objectness_masks, assignment)
    ...
    def get_bboxes(self,
        ...
        for prefix in prefixes:
            ...
            if use_iou_for_nms:
                B, num_proposal = bbox_preds[f'{prefix}iou_scores'].shape[:2]
                _, indx = bbox_preds[f'{prefix}sem_scores'].reshape(-1, self.num_classes).max(dim=-1)
